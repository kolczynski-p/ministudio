...
def get_photos():
    result_json = '{"photos": ['
    gallery_photos = [json.dumps(P.make_dict())
                      for P in GalleryPhoto.query.all()]
...
def get_preview_image(id):
    preview_image = GalleryPhoto.query.filter_by(
        workId=id, galleryIndex=1).first()
...
def get_work_images():
    result_json = '{"photos": ['
    work_photos = [json.dumps(P.photoName)
                   for P in GalleryPhoto.query.filter_by(workId=1)]
